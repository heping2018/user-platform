...
public class DatabaseUserRepository implements UserRepository {
    ...
    protected boolean execute(String sql, Consumer<Throwable> exceptionHandler, Object... args){
        ...
        try {
            ...
            for (int i = 0; i < args.length; i++) {
                preparedStatementMethod(preparedStatement,args[i],i+1);
            }
    ...
    private PreparedStatement preparedStatementMethod(PreparedStatement preparedStatement,Object arg,Object index) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        if (arg == null){
            arg = "";
        }
        ...
        Class indexType = index.getClass();
        Class warpperIndex = wrapperToPrimitive(indexType);

        if (wrapperType == null) {
            wrapperType = argType;
        }
        if (warpperIndex == null){
            warpperIndex = indexType;
        }
        String methodName = preparedStatementMethodMappings.get(argType);
        Method method = PreparedStatement.class.getMethod(methodName, warpperIndex,wrapperType);
